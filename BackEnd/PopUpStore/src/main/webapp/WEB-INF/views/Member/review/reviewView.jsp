...
<body>
    <div class="container-custom">
                ...
                <tbody>
                    <c:forEach var="comment" items="${comments}">
					    <tr id="comment-${comment.commentId}">
					        ...
					        <td>
					            <c:choose>
					                <c:when test="${sessionScope.userNick == comment.userNick || sessionScope.userNick == 'Admin'}">
					                    <!-- 댓글 작성자 본인 또는 관리자 계정일 경우에만 삭제 버튼 출력 -->
					                    <button type="button" class="btn btn-danger btn-sm" onclick="confirmReviewCommentDelete(${comment.commentId})">삭제</button>
					                </c:when>
					                <c:otherwise>
					                    <!-- 댓글 작성자와 로그인 유저가 다를 경우 삭제 버튼 미출력 -->
					                    <span>-</span>
					                </c:otherwise>
					            </c:choose>
					        </td>
        ...
        <div class="text-center mt-5">
		    <c:choose>
		        <c:when test="${sessionScope.userNick == dto.userNick || sessionScope.userNick == 'Admin'}">
		            <!-- 작성자 본인 또는 관리자 계정일 경우 출력 -->
